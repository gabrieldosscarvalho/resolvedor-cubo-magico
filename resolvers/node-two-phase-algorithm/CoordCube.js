...
var parityMove = (CoordCubie.parityMove = [
...
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    for (var i = 0; i < N_URFtoDLF; i++) {
      URFtoDLF_Move[i] = new Int16Array(
        data.slice(i * N_MOVE * 2, i * N_MOVE * 2 + N_MOVE * 2)
      );
    }
  } else {
    ...
    var data = "";
    for (var i = 0; i < N_URFtoDLF; i++) {
      data += new Buffer(new Uint8Array(URFtoDLF_Move[i].buffer)).toString(
        "binary"
      );
    }
...
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    var data = "";
    for (var i = 0; i < N_URtoDF; i++) {
      data += new Buffer(new Uint8Array(URtoDF_Move[i].buffer)).toString(
        "binary"
      );
    }
    fs.writeFileSync(filename, data, "binary");
...
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    var data = "";
    for (var i = 0; i < N_URtoUL; i++) {
      data += new Buffer(new Uint8Array(URtoUL_Move[i].buffer)).toString(
        "binary"
      );
    }
    fs.writeFileSync(filename, data, "binary");
...
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    var data = "";
    for (var i = 0; i < N_UBtoDF; i++) {
      data += new Buffer(new Uint8Array(UBtoDF_Move[i].buffer)).toString(
        "binary"
      );
    }
    fs.writeFileSync(filename, data, "binary");
...
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    var data = fs.readFileSync(filename);
    for (var i = 0; i < 336; i++) {
      MergeURtoULandUBtoDF[i] = new Int16Array(
        data.slice(i * 336 * 2, i * 336 * 2 + 336 * 2)
      );
    }
  } else {
    ...
    for (var uRtoUL = 0; uRtoUL < 336; uRtoUL++) {
      for (var uBtoDF = 0; uBtoDF < 336; uBtoDF++) {
        MergeURtoULandUBtoDF[uRtoUL][uBtoDF] = CubieCube.getURtoDF(
          uRtoUL,
          uBtoDF
        );
      }
    ...
    for (var i = 0; i < 336; i++) {
      data += new Buffer(
        new Uint8Array(MergeURtoULandUBtoDF[i].buffer)
      ).toString("binary");
    }
    fs.writeFileSync(filename, data, "binary");
  }
...
var Slice_URFtoDLF_Parity_Prun = (CoordCubie.Slice_URFtoDLF_Parity_Prun =
  new Buffer((N_SLICE2 * N_URFtoDLF * N_PARITY) / 2));
(function () {
...
var Slice_URtoDF_Parity_Prun = (CoordCubie.Slice_URtoDF_Parity_Prun =
  new Buffer((N_SLICE2 * N_URtoDF * N_PARITY) / 2));
(function () {
...
var Slice_Twist_Prun = (CoordCubie.Slice_Twist_Prun = new Buffer(
      ...
      for (var i = 0; i < N_SLICE1 * N_TWIST; i++) {
        ...
        if (getPruning(Slice_Twist_Prun, i) === depth) {
          for (var j = 0; j < 18; j++) {
            ...
            if (
              getPruning(Slice_Twist_Prun, N_SLICE1 * newTwist + newSlice) ===
              0x0f
            ) {
              setPruning(
                Slice_Twist_Prun,
                N_SLICE1 * newTwist + newSlice,
                depth + 1
              );
              done++;
            }
...
var Slice_Flip_Prun = (CoordCubie.Slice_Flip_Prun = new Buffer(
  (N_SLICE1 * N_FLIP) / 2
));
(function () {
  ...
  if (fs.existsSync(filename)) {
    ...
    for (var i = 0; i < (N_SLICE1 * N_FLIP) / 2; i++) Slice_Flip_Prun[i] = -1;
    var depth = 0;
      ...
      for (var i = 0; i < N_SLICE1 * N_FLIP; i++) {
        ...
        if (getPruning(Slice_Flip_Prun, i) === depth) {
          for (var j = 0; j < 18; j++) {
            ...
            if (
              getPruning(Slice_Flip_Prun, N_SLICE1 * newFlip + newSlice) ===
              0x0f
            ) {
              setPruning(
                Slice_Flip_Prun,
                N_SLICE1 * newFlip + newSlice,
                depth + 1
              );
              done++;
            }
...
function setPruning(table, index, value) {
  if ((index & 1) == 0) table[~~(index / 2)] &= 0xf0 | value;
  else table[~~(index / 2)] &= 0x0f | (value << 4);
}
...
function getPruning(table, index) {
  if ((index & 1) === 0) return table[~~(index / 2)] & 0x0f;
  else return (table[~~(index / 2)] & 0xf0) >>> 4;
}
